...
class companyServices {
  ...
  async registerCompany(body) {
    ...
    const createCenter = await center.createCenter(dataCompany, transaction);
  ...
  async createCompany(body, transaction = null) {
    ...
    try {
      ...
      const query = `INSERT INTO booking_config.companys(
        company_name, no_identification, address, phone, country, city, email) VALUES ($1, $2, $3, $4, $5, $6, $7 ) RETURNING *`;
  ...
  async updateCompany(id, body) {
    ...
    try {
      const company_name = body.company_name;
      const no_identification = body.nit;
      const address = body.address;
      const phone = body.phone;
      const country = body.country;
      const city = body.city;
      ...
      const debugQuery = `
            ...
            SET company_name='${company_name}', no_identification='${no_identification}', address='${address}',
                phone='${phone}', country='${country}', city='${city}', email='${email}'
            WHERE company_id=${id}
            RETURNING *;
        `;
      ...
      const query = `UPDATE booking_config.companys
        SET  company_name=$1, no_identification=$2, address=$3, phone=$4, country=$5, city=$6, email=$7
